...
class BE:
    ...
    def initialize(self, eri_,compute_hf, restart=False):
        ...
        if not restart:
            ...
            assert (eri_ is not None) or (hasattr(self.mf, 'with_df')) or (self.integral_direct_DF), "Input mean-field object is missing ERI (mf._eri) or DF (mf.with_df) object AND integral direct DF routine was not requested. Please check your inputs."
            if eri_ is not None: # incore ao2mo using saved eri from mean-field calculation
                for I in range(self.Nfrag):
